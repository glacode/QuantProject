...
CloseToOpenCorrelationProvider.cs
Copyright (C) 2008 
Marco Milletti
...
{
  /// <summary>
  /// Class that provides close to open correlation's indexes two by two within a 
  /// given set of tickers
  /// </summary>
  [Serializable]
  public class CloseToOpenCorrelationProvider : CorrelationProvider
  {
		
		/// <summary>
		/// Creates the provider for the close to open correlation
		/// </summary>
		...
		/// of returns</param>
    public CloseToOpenCorrelationProvider(string[] tickersToAnalyze,
					 ...
					 benchmark)
    {
		...
		protected override void setEndOfDayDatesTime(DateTime startDate, DateTime endDate)
		{
			...
			this.lastEndOfDayDateTime = 
				new EndOfDayDateTime(endDate, EndOfDaySpecificTime.MarketOpen);
		}
		
		protected override void setReturnsManager()
		{
			CloseToOpenIntervals closeToOpenIntervals = 
				new CloseToOpenIntervals(this.firstEndOfDayDateTime, this.lastEndOfDayDateTime,
				                         this.benchmark);
			this.returnsManager = 
				new ReturnsManager(closeToOpenIntervals, 
