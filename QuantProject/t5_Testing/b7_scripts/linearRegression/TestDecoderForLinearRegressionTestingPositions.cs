...
TestDecoderForLinearRegressionTestingPositions.cs
Copyright (C) 2010
Glauco Siliprandi
...
{
	/// <summary>
	/// Test for the class DecoderForLinearRegressionTestingPositions
	/// </summary>
	[TestFixture]
	public class TestDecoderForLinearRegressionTestingPositions
	{
		...
		public void TestDecode()
		{
			DecoderForLinearRegressionTestingPositions
				decoderForLinearRegressionTestingPositions =
				new DecoderForLinearRegressionTestingPositions( 2 , new int[] { 1 , 1 , 1 } );
...
//					returnsManagerForTradingTickers , returnsManagerForSignalingTickers );
			...
			Assert.AreEqual( "TA" , meaning.TradingPortfolio[ 0 ].Ticker );
			Assert.IsTrue( meaning.TradingPortfolio[ 0 ].Weight > 0 ,
			              "meaning.TradingPortfolio[ 0 ].Weight > 0" );
			...
			Assert.AreEqual( 1 , signalingPortfolio0.Count , "signalingPortfolio0.Count" );
			Assert.AreEqual( "SE" , signalingPortfolio0[ 0 ].Ticker );
			...
			Assert.AreEqual( 1 , signalingPortfolio1.Count , "signalingPortfolio1.Count" );
			Assert.AreEqual( "SC" , signalingPortfolio1[ 0 ].Ticker );
			...
			Assert.AreEqual( 1 , signalingPortfolio2.Count , "signalingPortfolio2.Count" );
			Assert.AreEqual( "SA" , signalingPortfolio2[ 0 ].Ticker );
		...
		public void TestDuplicatedTickers()
		{
			...
			testingPositions =
				decoderForLinearRegressionTestingPositions.Decode(
					...
					eligibleTickersForSignaling , returnsManager , returnsManager );
			Assert.IsTrue( testingPositions is TestingPositionsForUndecodableEncoded ,
			              "same ticker for signaling (third and fourth)" );
			...
			testingPositions =
				decoderForLinearRegressionTestingPositions.Decode(
					...
					eligibleTickersForSignaling , returnsManager , returnsManager );
			Assert.IsTrue( testingPositions is TestingPositionsForUndecodableEncoded ,
			              "same ticker for signaling (fourth and fifth)" );
			...
			Assert.IsFalse( testingPositions is TestingPositionsForUndecodableEncoded ,
			              "same ticker for the first trading ticker and for the third signaling portfolio" );
