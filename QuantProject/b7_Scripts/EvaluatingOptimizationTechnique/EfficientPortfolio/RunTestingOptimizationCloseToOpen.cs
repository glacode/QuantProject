...
RunTestingOptimizationCloseToOpen.cs
Copyright (C) 2003 
Marco Milletti
...
namespace QuantProject.Scripts.EvaluatingOptimizationTechnique.EfficientPortfolio
{
	...
	/// for the efficient close to open portfolio
	/// </summary>
  [Serializable]
  public class RunTestingOptimizationCloseToOpen
  {
    ...
    public RunTestingOptimizationCloseToOpen(string tickerGroupID, int numberOfEligibleTickers,
    ...
    {
      SelectorByGroup temporizedGroup = new SelectorByGroup(this.tickerGroupID, date);
  		...
  		
    {
      ...
      foreach(string tickerCode in ((GenomeMeaning)genome.Meaning).Tickers)
      {
        ...
        //returnValue +=
//          (tickerQuotes.GetFirstValidAdjustedOpen(dateOutOfSample.AddDays(1))/
        //  tickerQuotes.GetFirstValidAdjustedClose(dateOutOfSample) - 1.0)*coefficient;
	 			//
