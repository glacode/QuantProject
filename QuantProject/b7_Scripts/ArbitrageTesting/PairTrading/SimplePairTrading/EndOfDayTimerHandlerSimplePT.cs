...
namespace QuantProject.Scripts.ArbitrageTesting.PairTrading.SimplePairTrading
{
  ...
  /// </summary>
  [Serializable]
  public class EndOfDayTimerHandlerSimplePT
  {
    ...
    private bool marketCloseEventHandler_isCurrentGainGoodEnoughOrStopLossConditionReached(EndOfDayDateTime currentEndOfDayDateTime)
    {
    ...
    private void marketCloseEventHandler_openPositions(EndOfDayDateTime currentEndOfDayDateTime,
                                                       IndexBasedEndOfDayTimer timer)
    {
      ...
      //potential tickers for pair trading have been set
      {
          ...
          this.account.Portfolio.Count == 0; i++)
        {
          ...
          if(Math.Abs(this.marketCloseEventHandler_openPositions_getGap(i, timer.GetPreviousDateTime(),
            timer.GetCurrentTime().DateTime)) >= (float)this.maxLevelForNormalGap)
      ...
      Object sender , EndOfDayTimingEventArgs endOfDayTimingEventArgs )
    {
      ...
      //portfolio is empty
      {
        ...
        this.marketCloseEventHandler_openPositions(endOfDayTimingEventArgs.EndOfDayDateTime,
      ...
      else // portfolio is not empty
      {
        ...
        if(this.counterForDaysWithPositions == this.currentNumDaysForGapComputation ||
           this.marketCloseEventHandler_isCurrentGainGoodEnoughOrStopLossConditionReached(endOfDayTimingEventArgs.EndOfDayDateTime))
