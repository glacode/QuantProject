...
namespace QuantProject.Scripts.TickerSelectionTesting.OTC_CTO.OTC_CTO_EOD
{
	/// <summary>
	/// Implements the open to close - clos to open strategy (buy a portfolio at open
	/// and sell it at close and then reverse positions during night)
	/// using EOD quotes and reducing at minimum the number
	/// of trades required for playing it
	/// </summary>
	[Serializable]
	public class OTC_CTO_EODStrategy : IStrategyForBacktester
	{
...
//		private bool optimalPositionsHaveBeenUpdated;
    ...
    protected Time nearToClosingTimeTo;
		...
		protected Portfolio currentCTOPortfolio;
		protected Portfolio currentOTCPortfolio;
		...
		private void otc_cto_EODStrategy_commonInitialization(IEligiblesSelector eligiblesSelector,
		...
		public OTC_CTO_EODStrategy(IEligiblesSelector eligiblesSelector,
		                       int minimumNumberOfEligiblesForValidOptimization,
			...
			
		{
			...
			this.otc_cto_EODStrategy_commonInitialization(eligiblesSelector,
		...
		#region newDateTimeEventHandler_openCTOPortfolio
		
		private void newDateTimeEventHandler_openCTOPortfolio(Time currentDailyTime)
		{
			if(	this.chosenOTCPositions != null &&
			    this.allTickersAreExchanged( this.now(),
			        this.chosenOTCPositions[0].WeightedPositions.SignedTickers.Tickers)
//			    &&
...
//			  )
			{
				try
				{
					WeightedPositions oppositeWeightedPositions = 
						this.chosenOTCPositions[0].WeightedPositions.Opposite;
					
					AccountManager.OpenPositions( oppositeWeightedPositions,
					                             	this.account );
					this.currentCTOPortfolio = this.account.Portfolio;
		...
		#endregion newDateTimeEventHandler_openCTOPortfolio
		
		#region newDateTimeEventHandler_openOTCPortfolio
		
		private void newDateTimeEventHandler_openOTCPortfolio(Time currentDailyTime)
		{
			if(	this.chosenOTCPositions != null &&
			    this.allTickersAreExchanged( this.now(),
			    this.chosenOTCPositions[0].WeightedPositions.SignedTickers.Tickers) )
			{
				try
				{
					this.currentOTCPortfolio = new Portfolio();
					foreach(Position position in this.currentCTOPortfolio.Values)
					{
						Position newOTCPosition = 
							new Position(position.Instrument, - 2 * position.Quantity);
						OrderType orderTypeForNewOTCPosition;
						if(position.IsLong)
							orderTypeForNewOTCPosition = OrderType.MarketSellShort;
						else//position.IsShort
							orderTypeForNewOTCPosition = OrderType.MarketBuy;
						Order order = new Order(orderTypeForNewOTCPosition,
						                        position.Instrument,
						                        2 * Math.Abs(position.Quantity));
						this.account.AddOrder(order);
						this.currentOTCPortfolio.Add(position.Instrument.Key, newOTCPosition);
					}
					this.lastEntryTime = currentDailyTime;
					this.previousAccountValue = this.account.GetMarketValue();
				}
		...
		#endregion newDateTimeEventHandler_openOTCPortfolio
		...
		{
			if(	this.currentOTCPortfolio != null &&
			    this.allTickersAreExchanged( this.now(),
			    this.chosenOTCPositions[0].WeightedPositions.SignedTickers.Tickers) )
			{
				try
				{
					foreach(Position position in this.currentOTCPortfolio.Values)
					{
						OrderType orderTypeForClosingOTCPosition;
						if(position.IsLong)
							orderTypeForClosingOTCPosition = OrderType.MarketSellShort;
						else//position.IsShort
							orderTypeForClosingOTCPosition = OrderType.MarketBuy;
						Order order = new Order(orderTypeForClosingOTCPosition,
						                        position.Instrument,
						                        Math.Abs(position.Quantity));
						this.account.AddOrder(order);
					}
					this.currentOTCPortfolio = null;
					this.previousAccountValue = this.account.GetMarketValue();
				}
				catch(Exception ex)
				{
		...
		#endregion newDateTimeEventHandler_closeOTCPortfolio
		
		#region newDateTimeEventHandler_closeBothPortfolios
		
		private void newDateTimeEventHandler_closeBothPortfolios(Time currentDailyTime)
		{
			if(	allTickersAreExchanged( this.now(), AccountManager.GetTickersInOpenedPositions(this.account) ) )
			{
				try
		...
		#endregion newDateTimeEventHandler_closeBothPortfolios
...
//																	this.stopLossConditionReached;
			if( this.account.Portfolio.Count > 0 && nearToOpeningTime )
				this.newDateTimeEventHandler_openOTCPortfolio(currentTime);
			
			if( this.account.Portfolio.Count > 0 && nearToClosingTime )
			{
				if( this.optimalTestingPositionsAreToBeUpdated() )
				{
					this.newDateTimeEventHandler_closeBothPortfolios(currentTime);
					this.updateTestingPositions( dateTime );
					this.lastOptimizationDateTime = this.now();
				}
				else
					this.newDateTimeEventHandler_closeOTCPortfolio(currentTime);
			}
			
			if( this.account.Portfolio.Count == 0 && nearToClosingTime )
				this.newDateTimeEventHandler_openCTOPortfolio(currentTime);
		}
